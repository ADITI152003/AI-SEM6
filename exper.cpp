...
{
    int score = 0;
    for (int i = 0; i < MENTAL_HEALTH_QUESTIONS.size(); i++)
    {
        cout << MENTAL_HEALTH_QUESTIONS[i] << "y/n"<<endl;
        string ans;
        getline(cin, ans);
        while (ans != "y" && ans != "n")
        {
    ...
    if (score >= MENTAL_HEALTH_THRESHOLD["Severe"])
    {
    ...
    else if (score >= MENTAL_HEALTH_THRESHOLD["Moderate"])
    {
    ...
    else if (score >= MENTAL_HEALTH_THRESHOLD["Mild"])
    {
